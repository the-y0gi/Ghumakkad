...
const PopularExperienceSection = ({ groupedExperiences = {}, searchData = {} }) => {
  const navigate = useNavigate();
  const fallbackImage = "https://via.placeholder.com/300x200?text=No+Image";

  const handleClick = (expId) => {
    const params = new URLSearchParams({
      guests: searchData.guests || 1,
    }).toString();
    navigate(`/experience/${expId}?${params}`);
  };

  return (
    <div className="space-y-8 sm:space-y-16 py-4 sm:py-8 px-4 sm:px-0">
      {Object.entries(groupedExperiences).map(([state, experiences]) => (
            ...
            {experiences.map((exp, index) => (
              ...
              >
                ...
                <div className="relative h-40 sm:h-48 lg:h-56 overflow-hidden rounded-t-xl sm:rounded-t-2xl">
                  <img
                    src={exp.images?.[0] || fallbackImage}
                    alt={exp.title}
...
export default PopularExperienceSection;
